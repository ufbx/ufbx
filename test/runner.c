...
static const ufbxt_fuzz_check g_fuzz_checks[] = {
        { "maya_pivots", 6102, 2, 33, "total <= ator->max_size - ator->current_size" },
        { "maya_pivots", 6101, 272, 255, "str || length == 0" },
        { "maya_pivots", 6101, 193, 0, "uc->read_fn" },
        { "maya_pivots", 6101, 193, 0, "ufbxi_read_bytes(uc, (size_t)to_skip)" },
        { "maya_pivots", 6101, 2900, 1, "Bad multivalue array type" },
        { "maya_pivots", 6101, 3667, 1, "Bad multivalue array type" },
        { "maya_pivots", 6101, 25, 255, "num_values64 <= (uint64_t)uc->opts.max_node_values" },
        { "maya_pivots", 7501, 19426, 255, "size <= uc->opts.max_array_size" },
        { "maya_pivots", 7501, 3334, 117, "encoded_size == decoded_data_size" },
        { "maya_pivots", 7501, 19427, 1, "res == (ptrdiff_t)decoded_data_size" },
        { "maya_pivots", 7501, 14403, 255, "Bad array encoding" },
        { "maya_pivots", 6101, 2900, 1, "ufbxi_binary_parse_multivalue_array(uc, dst_type, arr_d..." },
        { "maya_pivots", 6101, 272, 255, "ufbxi_push_string_place_str(uc, &vals[i].s)" },
        { "maya_pivots", 6101, 265, 109, "ufbxi_skip_bytes(uc, encoded_size)" },
        { "maya_pivots", 6101, 24, 213, "Bad values type" },
        { "maya_pivots", 6101, 197, 24, "offset <= values_end_offset" },
        { "maya_pivots", 6101, 193, 0, "ufbxi_skip_bytes(uc, values_end_offset - offset)" },
        { "maya_pivots", 6101, 197, 26, "current_offset == end_offset" },
        { "maya_pivots", 6101, 193, 0, "ufbxi_binary_parse_node(uc, depth + 1, parse_state, &en..." },
        { "maya_pivots", 6102, 11, 49, "end == token->str_data + token->str_len - 1" },
        { "maya_pivots", 6102, 24, 102, "end == token->str_data + token->str_len - 1" },
        { "maya_pivots", 6102, 8, 0, "c != '\\0'" },
        { "maya_pivots", 6102, 8, 0, "ufbxi_ascii_next_token(uc, &ua->token)" },
        { "maya_pivots", 6102, 11, 47, "ufbxi_ascii_next_token(uc, &ua->token)" },
        { "maya_pivots", 6101, 1, 76, "ufbxi_ascii_accept(uc, UFBXI_ASCII_NAME)" },
        { "maya_pivots", 7502, 6922, 255, "ufbxi_ascii_accept(uc, UFBXI_ASCII_INT)" },
        { "maya_pivots", 7502, 7681, 67, "ufbxi_ascii_accept(uc, UFBXI_ASCII_NAME)" },
        { "maya_pivots", 7502, 6669, 10, "ufbxi_ascii_accept(uc, '}')" },
        { "maya_pivots", 6102, 8, 0, "ufbxi_ascii_parse_node(uc, depth + 1, parse_state, &end..." },
        { "maya_pivots", 6102, 8, 0, "ufbxi_ascii_parse_node(uc, 0, state, p_end, buf, true)" },
        { "maya_pivots", 6101, 28, 175, "ufbxi_binary_parse_node(uc, 0, state, p_end, buf, true)" },
        { "maya_pivots", 6101, 1, 76, "ufbxi_ascii_parse_node(uc, 0, UFBXI_PARSE_ROOT, &end, &..." },
        { "maya_pivots", 6101, 24, 213, "ufbxi_binary_parse_node(uc, 0, UFBXI_PARSE_ROOT, &end, ..." },
        { "maya_pivots", 6102, 2, 33, "ufbxi_grow_array(&uc->ator_tmp, &uc->top_nodes, &uc->to..." },
        { "maya_pivots", 6101, 521, 14, "ufbxi_parse_toplevel_child_imp(uc, state, &uc->tmp, &en..." },
        { "maya_pivots", 6101, 28, 175, "ufbxi_parse_toplevel_child_imp(uc, state, &uc->tmp_pars..." },
        { "maya_pivots", 6101, 28, 175, "ufbxi_parse_toplevel_child(uc, &child)" },
        { "maya_pivots", 7501, 3911, 255, "ufbxi_parse_toplevel_child(uc, &child)" },
        { "maya_pivots", 6101, 2902, 255, "ufbxi_get_val2(node, \"SC\", &prop->name, (char**)&type..." },
        { "maya_pivots", 7501, 5186, 96, "ufbxi_get_val_at(node, val_ix++, 'C', (char**)&subtype_..." },
        { "maya_pivots", 6101, 2902, 255, "ufbxi_read_property(uc, prop_node, prop, version)" },
        { "maya_pivots", 6101, 516, 73, "ufbxi_parse_toplevel_child(uc, &object)" },
        { "maya_pivots", 7501, 5900, 0, "ufbxi_get_val1(props, \"S\", &tmpl->sub_type)" },
        { "maya_pivots", 7501, 5185, 0, "ufbxi_get_val1(object, \"C\", (char**)&tmpl->type)" },
        { "maya_pivots", 7501, 520, 98, "ufbxi_read_properties(uc, props, &tmpl->props, &uc->tmp..." },
        { "maya_pivots", 6101, 4188, 255, "data" },
        { "maya_pivots", 6101, 9985, 106, "ufbxi_find_val1(node, ufbxi_MappingInformationType, \"C..." },
        { "maya_pivots", 6101, 9993, 14, "Invalid mapping" },
        { "maya_pivots", 6102, 3588, 255, "Invalid mapping" },
        { "maya_pivots", 6102, 1097, 46, "vertices && indices" },
        { "maya_pivots", 6101, 4691, 255, "index_ix >= 0 && (size_t)index_ix < mesh->num_indices" },
        { "maya_pivots", 6101, 3924, 255, "ix < mesh->num_vertices" },
        { "maya_pivots", 6101, 4188, 255, "ufbxi_read_vertex_element(uc, mesh, n, &mesh->vertex_no..." },
        { "maya_pivots", 6101, 5121, 255, "ufbxi_read_vertex_element(uc, mesh, n, &set->vertex_uv...." },
        { "maya_pivots", 6101, 3924, 255, "ufbxi_read_geometry(uc, node, &geom_obj)" },
        { "maya_pivots", 7501, 5131, 255, "ufbxi_check_string(*type)" },
        { "maya_pivots", 6101, 11788, 0, "ufbxi_check_string(*name)" },
        { "maya_pivots", 6101, 772, 55, "ufbxi_parse_toplevel_child(uc, &node)" },
        { "maya_pivots", 6101, 11788, 0, "ufbxi_split_type_and_name(uc, type_and_name, &type_str,..." },
        { "maya_pivots", 7501, 3842, 255, "ufbxi_read_properties(uc, node, &object.props, &uc->tmp..." },
        { "maya_pivots", 6101, 2902, 255, "ufbxi_read_properties(uc, node, &object.props, &uc->res..." },
        { "maya_pivots", 6101, 3924, 255, "ufbxi_read_model(uc, node, &object)" },
        { "maya_pivots", 7501, 2564, 71, "ufbxi_read_geometry(uc, node, &object)" },
        { "maya_pivots", 6101, 6401, 255, "ufbxi_parse_toplevel_child(uc, &node)" },
        { "maya_pivots", 6101, 14636, 113, "ufbxi_parse_toplevel_child(uc, &node)" },
        { "maya_pivots", 6102, 10792, 80, "ufbxi_get_val_at(node, 3, 'C', (char**)&prop)" },
        { "maya_pivots", 6101, 1, 76, "ufbxi_parse_toplevel(uc, ufbxi_FBXHeaderExtension)" },
        { "maya_pivots", 6101, 28, 175, "ufbxi_read_header_extension(uc)" },
        { "maya_pivots", 6101, 1, 0, "ufbxi_parse_toplevel(uc, ufbxi_Documents)" },
        { "maya_pivots", 7501, 3911, 255, "ufbxi_read_document(uc)" },
        { "maya_pivots", 6101, 204, 110, "ufbxi_parse_toplevel(uc, ufbxi_Definitions)" },
        { "maya_pivots", 6101, 516, 73, "ufbxi_read_definitions(uc)" },
        { "maya_pivots", 6101, 769, 52, "ufbxi_parse_toplevel(uc, ufbxi_Objects)" },
        { "maya_pivots", 6101, 772, 55, "ufbxi_read_objects(uc)" },
        { "maya_pivots", 6101, 14370, 255, "ufbxi_parse_toplevel(uc, ufbxi_Connections)" },
        { "maya_pivots", 6101, 14636, 113, "ufbxi_read_connections(uc)" },
        { "maya_pivots", 6101, 15919, 110, "ufbxi_parse_toplevel(uc, ufbxi_Takes)" },
        { "maya_pivots", 6101, 6401, 255, "ufbxi_read_takes(uc)" },
        { "maya_pivots", 6101, 1, 76, "ufbxi_read_root(uc)" },
};
