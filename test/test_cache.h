...
static void ufbxt_test_sine_cache(ufbxt_diff_error *err, const char *path, double begin, double end, double err_threshold)
{
	...
	ufbx_geometry_cache *cache = ufbx_load_geometry_cache(path, NULL, NULL);
	ufbxt_assert(cache);
		...
		if (!strcmp(channel->name.data, "pCubeShape1")) {
			...
			for (double time = begin; time <= end + 0.0001; time += 0.1/24.0) {
				...
				for (size_t i = 0; i < num_verts; i++) {
					...
					double sx = sin(v.y + t*pi2) * 0.25;
					double vx = v.x;
					vx += vx > 0.0 ? -0.5 : 0.5;
					ufbxt_assert_close_real(err, vx/err_threshold, sx/err_threshold);
				}
			}
		}
	}
...
#if UFBXT_IMPL
{
	...
	ufbxt_test_sine_cache(err, "caches/sine_mcmf_undersample/cache.xml", 1.0/24.0, 29.0/24.0, 0.01);
	ufbxt_test_sine_cache(err, "caches/sine_mcsd_oversample/cache.xml", 1.0/24.0, 29.0/24.0, 0.01);
	ufbxt_test_sine_cache(err, "caches/sine_mxmd_oversample/cache.xml", 11.0/24.0, 19.0/24.0, 0.01);
	ufbxt_test_sine_cache(err, "caches/sine_mxsf_regular/cache.xml", 1.0/24.0, 29.0/24.0, 0.01);
}
#endif
