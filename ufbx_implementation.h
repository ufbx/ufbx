...
typedef enum {
	...
	UFBXI_GENERATED_INDEX_CONSECUTIVE,
	// Indices 0,0,0,0,0,0,0,...
	UFBXI_GENERATED_INDEX_ZERO,

	UFBXI_NUM_GENERATED_INDEX_TYPES,
} ufbxi_generated_index_type;
...
typedef struct {
	...
	ufbxi_base_context bc;
} ufbxi_scene_imp;

typedef struct ufbxi_context {
	ufbxi_base_context base;
	...
	// Generated indices
	size_t max_generated_indices[UFBXI_NUM_GENERATED_INDEX_TYPES];
...
static void ufbxi_do_error_common(ufbxi_base_context *bc, uint32_t line)
{
	bc->failed = 1;
	ufbx_error *err = bc->error;
	if (err) {
		err->source_line = line;

		ufbxi_context *uc = bc->uc;
		if (uc) {
...
static int ufbxi_do_error(ufbxi_base_context *bc, uint32_t line, const char *desc)
{
	if (bc->failed) return 0;
	ufbxi_do_error_common(bc, line);
	if (bc->error) {
		size_t length = strlen(desc);
		if (length > UFBX_ERROR_DESC_MAX_LENGTH) length = UFBX_ERROR_DESC_MAX_LENGTH;
		memcpy(bc->error->desc, desc, length);
		bc->error->desc[length] = '\0';
	}
	return 0;
}

static int ufbxi_do_errorf(ufbxi_base_context *bc, uint32_t line, const char *fmt, ...)
{
	if (bc->failed) return 0;
	ufbxi_do_error_common(bc, line);
	if (bc->error) {
		va_list args;
		va_start(args, fmt);
		vsnprintf(bc->error->desc, sizeof(bc->error->desc), fmt, args);
		va_end(args);
	}
	return 0;
}

#define ufbxi_base_error(bc, desc) ufbxi_do_error((bc), __LINE__, (desc))
#define ufbxi_base_errorf(bc, fmt, ...) ufbxi_do_errorf((bc), __LINE__, (fmt), __VA_ARGS__)

#define ufbxi_error(uc, desc) ufbxi_do_error(&(uc)->base, __LINE__, (desc))
#define ufbxi_errorf(uc, fmt, ...) ufbxi_do_errorf(&(uc)->base, __LINE__, (fmt), __VA_ARGS__)
...
static void *ufbxi_push_size_uninit(ufbxi_context *uc, size_t size, uint32_t line)
{
	...
	if (uc->result_size - pos >= size) {
		...
		if (!page) {
			ufbxi_do_errorf(&uc->base, line, "Failed to allocate memory: %zu bytes", page_size);
			return NULL;
		}
...
static int ufbxi_temp_is_empty(ufbxi_context *uc, ufbxi_temp_stack_index index)
{
	ufbx_assert((unsigned)index < UFBXI_NUM_TEMP_STACKS);
	ufbxi_temp_stack *stack = &uc->temp_stacks[index];
	return stack->page_index == 0 && stack->pages[0].offset == 0;
}
...
static void *ufbxi_temp_push_size_uninit(ufbxi_context *uc, ufbxi_temp_stack_index index, size_t size, uint32_t line)
{
	...
	ufbxi_do_errorf(&uc->base, line, "Temp stack overflow: %d", index);
	return NULL;
}
...
static ufbx_string ufbxi_push_string_size(ufbxi_context *uc, ufbx_string str, uint32_t line)
{
	...
	if (str.length == 0 || uc->base.failed) {
...
#define ufbxi_temp_pop(uc, index, type) (type*)ufbxi_temp_pop_size((uc), (index), 1, sizeof(type))
#define ufbxi_temp_pop_n(uc, index, type, n) (type*)ufbxi_temp_pop_size((uc), (index), (n), sizeof(type))
...
static int ufbxi_convert_multivalue_array(ufbxi_context *uc, ufbxi_array *dst, char dst_type)
{
	...
	while (uc->focused_node.next_value_pos < uc->focused_node.child_begin_pos) {
		char src_type = uc->base.data[uc->focused_node.next_value_pos];
		uint32_t size;
		switch (src_type) {
		case 'I': size = 4; break;
		case 'L': size = 8; break;
...
//   Numbers: "I" u/int32_t  "L" u/int64_t  "B" char (bool)  "F" float  "D" double  "U" ufbx_real
//   Data: "SR" ufbx_string  "ilfdbu" ufbxi_val_array (matching upper-case type)
//   Misc: "." (ignore)  "?" ufbxi_any_value  "_" ufbxi_val_array (any type)
...
static int ufbxi_parse_value(ufbxi_context *uc, char dst_type, void *dst)
{
	...
	if (uc->base.size - uc->focused_node.next_value_pos < 13) {
	...
	char src_type = uc->base.data[pos];
	const char *src = uc->base.data + pos + 1;
	...
	if (dst_type == '.') {
		...
		dst_type = sizeof(ufbx_real) == sizeof(double) ? 'D' : 'F';
	} else if (dst_type == 'u') {
		dst_type = sizeof(ufbx_real) == sizeof(double) ? 'd' : 'f';
	}
	...
	if (val_class == UFBXI_VAL_INT) {
		...
		case 'i': case 'l': case 'f': case 'd': case 'b': case '_':
		...
		str->data = uc->base.data + val_data_offset;
...
static int ufbxi_decode_multivalue_array(ufbxi_base_context *bc, ufbxi_array *arr, void *dst)
{
	if (arr->encoding != UFBXI_ENCODING_MULTIVALUE) {
		return ufbxi_base_error(bc, "Internal: Bad multivalue encoding");
	}

	// HACK: Create a new virtual `ufbxi_context` for the multivalue
	...
	default:
		return ufbxi_base_error(bc, "Internal: Bad dst element type");
	}
	...
	if (uc.focused_node.next_value_pos != uc.focused_node.child_begin_pos) {
		return ufbxi_base_error(bc, "Multivalue array overrun");
	}
	if (dst_ptr != (char*)dst + arr->num_elements * dst_elem_size) {
		return ufbxi_base_error(bc, "Internal: Multivalue array read failed");
	}
...
static int ufbxi_decode_array(ufbxi_base_context *bc, ufbxi_array *arr, void *dst)
{
	uint32_t elem_size;
	switch (arr->src_type) {
	...
	default: return ufbxi_base_error(bc, "Internal: Invalid array type");
	}

	size_t arr_size = arr->num_elements * elem_size;

	if (arr->dst_type == arr->src_type) {
		const void *src = bc->data + arr->data_offset;
		...
		case UFBXI_ENCODING_BASIC:
			if (arr->encoded_size != arr_size) {
				return ufbxi_base_errorf(bc, "Array size mismatch, encoded %u bytes, decoded %u bytes",
		...
		case UFBXI_ENCODING_DEFLATE: {
			...
			if (res != arr_size) {
				if (res < 0) {
					return ufbxi_base_errorf(bc, "Deflate error: %d", (int)res);
				} else {
					return ufbxi_base_error(bc, "Deflate size mismatch");
				}
			}
		} break;
		case UFBXI_ENCODING_MULTIVALUE:
			// Early return: Defer to multivalue implementation.
			return ufbxi_decode_multivalue_array(bc, arr, dst);
		default: return ufbxi_base_error(bc, "Internal: Bad array encoding");
		}
	} else {
		const char *src_ptr;
		const void *src = bc->data + arr->data_offset;
		...
		case UFBXI_ENCODING_BASIC:
			...
			break;
		case UFBXI_ENCODING_DEFLATE: {
			size_t buf_size = bc->decompress_buffer_size;
			if (buf_size < arr_size) {
				buf_size = buf_size * 2 > arr_size ? buf_size * 2 : arr_size;
				bc->decompress_buffer_size = buf_size;
				void *buf = realloc(bc->decompress_buffer, buf_size);
				if (!buf) {
					return ufbxi_base_errorf(bc, "Failed to allocate decompression buffer: %zu bytes", buf_size);
				}
				bc->decompress_buffer = buf;
			}
			ptrdiff_t res = ufbx_inflate(bc->decompress_buffer, arr_size, src, arr->encoded_size);
			if (res != arr_size) {
				if (res < 0) {
					return ufbxi_base_errorf(bc, "Deflate error: %d", (int)res);
				} else {
			...
			src_ptr = (const char*)bc->decompress_buffer;
		} break;
		case UFBXI_ENCODING_MULTIVALUE:
			// Multivalue arrays should always have `src_type == dst_type`.
			return ufbxi_base_error(bc, "Internal: Multivalue array has invalid type");
		default: return ufbxi_base_error(bc, "Internal: Bad array encoding");
		}
		...
		if (arr->dst_type == 'i') {
			...
			return ufbxi_base_error(bc, "Internal: Unexpected array type");
		}

		if (failed) {
			return ufbxi_base_errorf(bc, "Bad array conversion: '%c' -> '%c'",
...
static int ufbxi_parse_node(ufbxi_context *uc, uint64_t pos, ufbxi_node *node)
{
	...
	if (uc->version >= 7500 || uc->from_ascii) {
		if (pos > uc->base.size - 25) {
			return ufbxi_error(uc, "Internal: Trying to read node out of bounds");
		}
		end_pos = ufbxi_read_u64(uc->base.data + pos + 0);
		values_len = ufbxi_read_u64(uc->base.data + pos + 16);
		name_len = ufbxi_read_u8(uc->base.data + pos + 24);
		name_pos = pos + 25;
	} else {
		if (pos > uc->base.size - 13) {
			return ufbxi_error(uc, "Internal: Trying to read node out of bounds");
		}
		end_pos = ufbxi_read_u32(uc->base.data + pos + 0);
		values_len = ufbxi_read_u32(uc->base.data + pos + 8);
		name_len = ufbxi_read_u8(uc->base.data + pos + 12);
		name_pos = pos + 13;
	}
	...
	if (value_pos < name_pos || value_pos > uc->base.size) {
		return ufbxi_error(uc, "Node name out of bounds");
	}
	if (child_pos < value_pos || child_pos > uc->base.size) {
		return ufbxi_error(uc, "Node values out of bounds");
	}
	if (end_pos < child_pos || end_pos > uc->base.size) {
	...
	node->name.data = uc->base.data + name_pos;
...
typedef enum {
	UFBXI_ELEMENT_BY_UNKNOWN,
	UFBXI_ELEMENT_BY_VERTEX,
	UFBXI_ELEMENT_BY_POLYGON,
	UFBXI_ELEMENT_BY_POLYGON_VERTEX,
	UFBXI_ELEMENT_BY_EDGE,
	UFBXI_ELEMENT_BY_ALL_SAME,

	UFBXI_NUM_ELEMENT_MAPPINGS,
} ufbxi_element_mapping;

typedef enum {
	UFBXI_ELEMENT_UNKNOWN,
	UFBXI_ELEMENT_POSITION,
	UFBXI_ELEMENT_NORMAL,
	UFBXI_ELEMENT_BINORMAL,
	UFBXI_ELEMENT_TANGENT,
	UFBXI_ELEMENT_VERTEX_COLOR,
	UFBXI_ELEMENT_UV,
	UFBXI_ELEMENT_EDGE_SMOOTHING,
	UFBXI_ELEMENT_EDGE_CREASE,
	UFBXI_ELEMENT_FACE_SMOOTHING,
	UFBXI_ELEMENT_FACE_MATERIAL,

	UFBXI_NUM_ELEMENT_TYPES,
} ufbxi_element_type;

typedef enum {
	UFBXI_ELEMENT_KIND_UNKNOWN,
	UFBXI_ELEMENT_KIND_VERTEX,
	UFBXI_ELEMENT_KIND_EDGE,
	UFBXI_ELEMENT_KIND_FACE,
} ufbxi_element_kind;
...
typedef struct {
	uint32_t hash;
	ufbx_string name;
	ufbxi_element_mapping mapping;
} ufbxi_element_mapping_map_entry;

typedef struct {
	uint32_t hash;
	ufbx_string name;
	ufbxi_element_type type;
} ufbxi_element_type_map_entry;
...
#define ufbxi_element_mapping_permute_hash(h) ((((h) * 1) >> 3) % 8)
static const ufbxi_element_mapping_map_entry ufbxi_element_mapping_map[8] = {
	{ 0x8f0b1802u, { "ByVertex", 8 }, UFBXI_ELEMENT_BY_VERTEX },
	{ 0u, { 0,0 }, UFBXI_ELEMENT_BY_UNKNOWN },
	{ 0u, { 0,0 }, UFBXI_ELEMENT_BY_UNKNOWN },
	{ 0x65f0ae19u, { "ByEdge", 6 }, UFBXI_ELEMENT_BY_EDGE },
	{ 0x03031be2u, { "ByVertice", 9 }, UFBXI_ELEMENT_BY_VERTEX },
	{ 0xf6b4bb28u, { "ByPolygonVertex", 15 }, UFBXI_ELEMENT_BY_POLYGON_VERTEX },
	{ 0xf07b3af2u, { "AllSame", 7 }, UFBXI_ELEMENT_BY_ALL_SAME },
	{ 0x5e66f4fau, { "ByPolygon", 9 }, UFBXI_ELEMENT_BY_POLYGON },
};
#define ufbxi_element_type_permute_hash(h) ((((h) * 5) >> 20) % 8)
static const ufbxi_element_type_map_entry ufbxi_element_type_map[8] = {
	{ 0xe5b43cf8u, { "Color", 5 }, UFBXI_ELEMENT_VERTEX_COLOR },
	{ 0xe4d152a2u, { "Tangent", 7 }, UFBXI_ELEMENT_TANGENT },
	{ 0xcbd54f80u, { "Material", 8 }, UFBXI_ELEMENT_FACE_MATERIAL },
	{ 0u, { 0,0 }, UFBXI_ELEMENT_UNKNOWN },
	{ 0x68f39512u, { "UV", 2 }, UFBXI_ELEMENT_UV },
	{ 0x58de2772u, { "Normal", 6 }, UFBXI_ELEMENT_NORMAL },
	{ 0xc0c90f07u, { "EdgeCrease", 10 }, UFBXI_ELEMENT_EDGE_CREASE },
	{ 0x3030b747u, { "Binormal", 8 }, UFBXI_ELEMENT_BINORMAL },
};
// End of generated code

typedef struct {
	const char *name;
} ufbxi_prop_type_info;

typedef struct {
	const char *name;
	size_t size;
} ufbxi_node_type_info;

typedef struct {
	ufbxi_element_kind kind;
	...
	size_t offset;
} ufbxi_element_type_info;

typedef struct {
	const char *name;
} ufbxi_element_mapping_info;

static const ufbxi_node_type_info ufbxi_prop_type_info_table[] = {
	{ "unknown" },
	{ "boolean" },
	{ "integer" },
	{ "number" },
...
ufbx_static_assert(prop_info_count, ufbxi_arraycount(ufbxi_prop_type_info_table) == UFBX_NUM_PROP_TYPES);

static const ufbxi_node_type_info ufbxi_node_type_info_table[] = {
	{ "unknown", sizeof(ufbx_node) },
	{ "model", sizeof(ufbx_model) },
	{ "mesh", sizeof(ufbx_mesh) },
	{ "material", sizeof(ufbx_node) },
	{ "texture", sizeof(ufbx_node) },
	{ "bone", sizeof(ufbx_node) },
	{ "skin", sizeof(ufbx_node) },
	{ "animation", sizeof(ufbx_node) },
	{ "animation_curve", sizeof(ufbx_node) },
	{ "animation_layer", sizeof(ufbx_node) },
	{ "attribute", sizeof(ufbx_node) },
};
ufbx_static_assert(node_info_count, ufbxi_arraycount(ufbxi_node_type_info_table) == UFBX_NUM_NODE_TYPES);

static const ufbxi_element_type_info ufbxi_element_type_info_table[] = {
	{ UFBXI_ELEMENT_KIND_UNKNOWN },
	{ UFBXI_ELEMENT_KIND_VERTEX, "", "", 'u', 3, sizeof(ufbx_vec3), offsetof(ufbx_mesh, vertex_position) }, 
	{ UFBXI_ELEMENT_KIND_VERTEX, "Normals", "NormalIndex", 'u', 3, sizeof(ufbx_vec3), offsetof(ufbx_mesh, vertex_normal) }, 
	{ UFBXI_ELEMENT_KIND_VERTEX, "Binormals", "BinormalIndex", 'u', 3, sizeof(ufbx_vec3), offsetof(ufbx_mesh, vertex_binormal) },
	{ UFBXI_ELEMENT_KIND_VERTEX, "Tangents", "TangentIndex", 'u', 3, sizeof(ufbx_vec3), offsetof(ufbx_mesh, vertex_tangent) },
	{ UFBXI_ELEMENT_KIND_VERTEX, "Colors", "ColorIndex", 'u', 4, sizeof(ufbx_vec4), offsetof(ufbx_mesh, vertex_color) },
	{ UFBXI_ELEMENT_KIND_VERTEX, "UV", "UVIndex", 'u', 2, sizeof(ufbx_vec2), offsetof(ufbx_mesh, vertex_uv) },
	{ UFBXI_ELEMENT_KIND_EDGE, "Smoothing", NULL, 'b', 1, sizeof(bool), offsetof(ufbx_mesh, edge_smoothing) }, 
	{ UFBXI_ELEMENT_KIND_EDGE, "EdgeCrease", NULL, 'u', 1, sizeof(ufbx_real), offsetof(ufbx_mesh, edge_crease) }, 
	{ UFBXI_ELEMENT_KIND_FACE, "Smoothing", NULL, 'b', 1, sizeof(bool), offsetof(ufbx_mesh, face_smoothing) }, 
	{ UFBXI_ELEMENT_KIND_FACE, "Materials", NULL, 'i', 1, sizeof(int32_t), offsetof(ufbx_mesh, face_material) }, 
};
ufbx_static_assert(element_info_count, ufbxi_arraycount(ufbxi_element_type_info_table) == UFBXI_NUM_ELEMENT_TYPES);

static const ufbxi_element_mapping_info ufbxi_element_mapping_info_table[] = {
	{ "(unknown)" },
	{ "ByVertex" },
	{ "ByPolygon" },
	{ "ByPolygonVertex" },
...
// Resolve an FBX node type name into an enum
...
// Resolve FBX element mapping type into an enum
static ufbxi_element_mapping ufbxi_get_element_mapping(ufbx_string type)
{
	uint32_t hash = ufbxi_hash_string(type);
	uint32_t index = ufbxi_element_mapping_permute_hash(hash);
	const ufbxi_element_mapping_map_entry *entry = &ufbxi_element_mapping_map[index];
	if (hash == entry->hash && ufbxi_streq_str(type, entry->name)) {
		return entry->mapping;
	}
	return UFBXI_ELEMENT_BY_UNKNOWN;
}

// Resolve FBX element mapping type into an enum
static ufbxi_element_type ufbxi_get_element_type(ufbx_string type, ufbxi_element_mapping mapping)
{
	uint32_t hash = ufbxi_hash_string(type);
	uint32_t index = ufbxi_element_type_permute_hash(hash);
	const ufbxi_element_type_map_entry *entry = &ufbxi_element_type_map[index];
	if (hash == entry->hash && ufbxi_streq_str(type, entry->name)) {
		return entry->type;
	} else if (ufbxi_streq(type, "Smoothing")) {
		if (mapping == UFBXI_ELEMENT_BY_EDGE) {
			return UFBXI_ELEMENT_EDGE_SMOOTHING;
		} else if (mapping == UFBXI_ELEMENT_BY_POLYGON) {
			return UFBXI_ELEMENT_FACE_SMOOTHING;
		}
	}
	return UFBXI_ELEMENT_UNKNOWN;
}
...
static int ufbxi_read_property(ufbxi_context *uc, ufbx_prop *prop, int version)
{
	...
	memset(prop->value_real, 0, sizeof(prop->value_real));
	...
	for (size_t i = 0; i < 4; i++) {
		...
		if (val.value_class == UFBXI_VAL_INT) {
			...
			prop->value_real[i] = (ufbx_real)val.value.i;
		} else if (val.value_class == UFBXI_VAL_FLOAT) {
			prop->value_int[i] = (int64_t)val.value.f;
			prop->value_real[i] = (ufbx_real)val.value.f;
...
static void ufbxi_hash_map_insert(ufbxi_hash_map_init_ctx *ctx, uint32_t hash, uint32_t index)
{
	...
	for (;;) {
		ufbxi_hash_map_entry *entry = &entries[(hash + scan) & mask];
		if (!entry->hash) {
			entry->hash = hash;
			entry->index = index;
			break;
...
static ufbxi_forceinline uint32_t ufbxi_hash_map_find(const void *hash_map, const char *data, size_t length, uint32_t hash, uint32_t *p_scan)
{
	...
	for (;;) {
		const ufbxi_hash_map_entry *entry = &entries[(hash + scan) & mask];
		if (entry->hash == hash) {
			*p_scan = scan + 1;
			return entry->index;
		} else if (!entry->hash) {
...
typedef struct {
	...
	ufbxi_element_type type;
	int32_t index;
	ufbxi_array data_array;
	ufbxi_array index_array;
} ufbxi_element_desc;

typedef struct {
	int32_t **p_indices;
	ufbxi_generated_index_type type;
} ufbxi_generated_index_ptr;

static int ufbxi_push_generated_index(ufbxi_context *uc, int32_t **p_indices, ufbxi_generated_index_type type, size_t count)
{
	ufbxi_generated_index_ptr *ptr = ufbxi_temp_push_uninit(uc, UFBXI_TEMP_GENERATED_INDICES, ufbxi_generated_index_ptr);
	if (!ptr) return 0;

	ptr->p_indices = p_indices;
	ptr->type = type;
	if (uc->max_generated_indices[type] < count) {
		uc->max_generated_indices[type] = count;
	}
	return 1;
}

static int ufbxi_push_generated_indices(ufbxi_context *uc, ufbx_vertex_element *p_elem, ufbx_vertex_element *p_set_elem, ufbxi_generated_index_type type, size_t count)
{
	if (p_elem) {
		if (!ufbxi_push_generated_index(uc, &p_elem->indices, type, count)) return 0;
	}
	if (p_set_elem) {
		if (!ufbxi_push_generated_index(uc, &p_set_elem->indices, type, count)) return 0;
	}
	return 1;
}
...
static int ufbxi_read_mesh(ufbxi_context *uc, ufbx_mesh *mesh)
{
	ufbxi_element_desc position_desc = { 0 };
	ufbxi_element_desc element_descs[UFBX_MAX_ELEMENTS_PER_MESH];
	size_t num_element_descs = 0;
	ufbxi_array index_array = { 0 };
	ufbxi_array edge_array = { 0 };

	int32_t lowest_uv_index = INT32_MAX;
	int32_t lowest_color_index = INT32_MAX;

	ufbx_string name;
	while (ufbxi_next_child(uc, &name)) {
		if (ufbxi_streq(name, "Vertices")) {
			// Vertex position data
			if (position_desc.type) return ufbxi_error(uc, "Multiple vertex position arrays");
			position_desc.type = UFBXI_ELEMENT_POSITION;
			position_desc.mapping = UFBXI_ELEMENT_BY_VERTEX;
			if (!ufbxi_parse_value(uc, 'u', &position_desc.data_array)) return 0;

		} else if (ufbxi_streq(name, "PolygonVertexIndex")) {
			if (!ufbxi_parse_value(uc, 'i', &index_array)) return 0;

		} else if (ufbxi_streq(name, "Edges")) {
			if (!ufbxi_parse_value(uc, 'i', &edge_array)) return 0;

		} else if (name.length > 12 && !memcmp(name.data, "LayerElement", 12)) {
			if (!ufbxi_enter_node(uc)) return 0;

			ufbx_string type_str = { name.data + 12, name.length - 12 };
			int32_t index = 0;
			if (ufbxi_has_next_value(uc)) {
				if (!ufbxi_parse_value(uc, 'I', &index)) return 0;
			}

			// The element type may depend on the mapping so query it first
			ufbx_string mapping_str;
			if (!ufbxi_find_value(uc, "MappingInformationType", 'S', &mapping_str)) return 0;
			ufbxi_element_mapping mapping = ufbxi_get_element_mapping(mapping_str);
			ufbxi_element_type type = ufbxi_get_element_type(type_str, mapping);

			if (mapping != UFBXI_ELEMENT_BY_UNKNOWN && type != UFBXI_ELEMENT_UNKNOWN) {
				if (num_element_descs >= UFBX_MAX_ELEMENTS_PER_MESH) {
					return ufbxi_error(uc, "Too many elements per mesh");
				}
				const ufbxi_element_type_info *info = &ufbxi_element_type_info_table[type];
				ufbxi_element_desc *desc = &element_descs[num_element_descs];
				memset(desc, 0, sizeof(ufbxi_element_desc));
				...
				ufbxi_find_value(uc, "Name", 'S', &desc->name);

				if (!ufbxi_find_value(uc, info->data_name, info->data_type, &desc->data_array)) {
					return ufbxi_errorf(uc, "Could not find vertex data array '%s'", info->data_name);
				} else {
					if (desc->data_array.num_elements == 0) continue;
				}

				if (info->index_name) {
					if (ufbxi_find_value(uc, info->index_name, 'i', &desc->index_array)) {
						if (desc->index_array.num_elements == 0) continue;
					}
				}

				// Count UV and color sets
				if (type == UFBXI_ELEMENT_UV) {
					if (mesh->uv_sets.size < index + 1) mesh->uv_sets.size = index + 1;
					if (lowest_uv_index > index) lowest_uv_index = index;
				} else if (type == UFBXI_ELEMENT_VERTEX_COLOR) {
					if (mesh->color_sets.size < index + 1) mesh->color_sets.size = index + 1;
					if (lowest_color_index > index) lowest_color_index = index;
				}
				num_element_descs++;
			}
	...
	if (index_array.data_offset == 0) return ufbxi_error(uc, "Mesh has no face index information");
	if (position_desc.type == UFBXI_ELEMENT_UNKNOWN) return ufbxi_error(uc, "Mesh has vertex information");
	if (position_desc.data_array.num_elements % 3 != 0) return ufbxi_error(uc, "Vertex array size is not divisible by 3");

	mesh->num_vertices = position_desc.data_array.num_elements / 3;
	mesh->num_indices = index_array.num_elements;
	mesh->num_edges = edge_array.num_elements;

	// Read vertex data
	{
		ufbx_vec3 *vertices = ufbxi_push_uninit_n(uc, ufbx_vec3, mesh->num_vertices);
		if (!vertices) return 0;
		if (!ufbxi_decode_array(&uc->base, &position_desc.data_array, vertices)) return 0;
		mesh->vertex_position.data = vertices;
	}

	// Generate faces from FBX face vertex index encoding, face indices are stored
	// in a contiguous array where negative numbers `~index` signify end-of-polygon.
	{
		int32_t *indices = ufbxi_push_uninit_n(uc, int32_t, mesh->num_indices);
		if (!indices) return 0;
		if (!ufbxi_decode_array(&uc->base, &index_array, indices)) return 0;
		...
		ufbx_face *faces = ufbxi_push_uninit_n(uc, ufbx_face, num_faces);
		if (!faces) return 0;

		// Read edge data while we have the un-negated indices
		if (edge_array.data_offset) {
			size_t num_edges = mesh->num_edges;
			int32_t *temp_indices = (int32_t*)ufbxi_temp_push_size_uninit(uc,
				UFBXI_TEMP_PRIMARY_RESULT, sizeof(int32_t) * num_edges, __LINE__);
			if (!temp_indices) return 0;
			if (!ufbxi_decode_array(&uc->base, &edge_array, temp_indices)) return 0;

			ufbx_edge *edges = ufbxi_push_uninit_n(uc, ufbx_edge, num_edges);
			if (!edges) return 0;
			mesh->edges = edges;
			...
			for (size_t i = 0; i < num_edges; i++) {
				...
				if (index_ix < 0 || index_ix >= mesh->num_indices) {
					return ufbxi_error(uc, "Edge index out of bounds");
				}
				...
				if (ix < 0) {
					...
					if (index_ix + 1 >= mesh->num_indices) {
						return ufbxi_error(uc, "Edge on unclosed final polygon");
					}
			...
			ufbxi_temp_pop_size(uc, UFBXI_TEMP_PRIMARY_RESULT, 1, sizeof(int32_t) * num_edges);
		}
		...
		for (int32_t *p_ix = indices; p_ix != indices_end; p_ix++) {
			...
			if (ix < 0) {
				ix = ~ix;
				*p_ix =  ix;
				faces[face_ix].index_begin = (int32_t)(p_face_begin - indices);
				faces[face_ix].num_indices = (int32_t)((p_ix - p_face_begin) + 1);
				face_ix++;
				p_face_begin = p_ix + 1;
			}
			if (ix >= mesh->num_vertices) return ufbxi_error(uc, "Index out of bounds");
		}
		ufbx_assert(face_ix == num_faces);
	}

	// Allocate storage for UV and color sets
	mesh->uv_sets.data = ufbxi_push_zero_n(uc, ufbx_uv_set, mesh->uv_sets.size);
	mesh->color_sets.data = ufbxi_push_zero_n(uc, ufbx_color_set, mesh->color_sets.size);
	if (uc->base.failed) return 0;

	size_t mesh_num_indices = mesh->num_indices;

	// Process the rest of the elements
	for (size_t i = 0; i < num_element_descs; i++) {
		ufbxi_element_desc *desc = &element_descs[i];
		const ufbxi_element_type_info *info = &ufbxi_element_type_info_table[desc->type];

		if (desc->data_array.num_elements % info->num_components != 0) {
			return ufbxi_errorf(uc, "Element '%s' %d is not divisible by data size %d",
				info->data_name, desc->index, info->num_components);
		}
		size_t num_elements = desc->data_array.num_elements / info->num_components;

		if (info->kind == UFBXI_ELEMENT_KIND_VERTEX) {
			int32_t invalid_index = uc->opts.allow_nonexistent_indices ? -1 : (int32_t)num_elements - 1;

			char *data = ufbxi_push_uninit_n(uc, char, num_elements * info->element_size);
			if (!data) return 0;
			if (!ufbxi_decode_array(&uc->base, &desc->data_array, data)) return 0;

			ufbx_vertex_element elem = { 0 };
			elem.data = data;

			// Resolve element (and set duplicate) element pointers as we may need to record
			// them for deferred index generation result pointers.
			ufbx_vertex_element *p_elem = (ufbx_vertex_element*)((char*)mesh + info->offset);
			ufbx_vertex_element *p_set_elem = NULL;
			if (desc->type == UFBXI_ELEMENT_UV) {
				mesh->uv_sets.data[desc->index].name = ufbxi_push_string(uc, desc->name);
				p_set_elem = &mesh->uv_sets.data[desc->index].vertex_uv.element;
				if (desc->index != lowest_uv_index) p_elem = NULL;
			} else if (desc->type == UFBXI_ELEMENT_VERTEX_COLOR) {
				mesh->color_sets.data[desc->index].name = ufbxi_push_string(uc, desc->name);
				p_set_elem = &mesh->uv_sets.data[desc->index].vertex_uv.element;
				if (desc->index == lowest_color_index) p_elem = NULL;
			}

			if (desc->index_array.data_offset) {
				size_t num_indices = desc->index_array.num_elements;

				switch (desc->mapping) {

				case UFBXI_ELEMENT_BY_VERTEX:
				{
					// Indexed by vertex: We need to temporarily decode the vertex
					// mapping and follow through the position mapping to get the
					// final indices.
					elem.indices = ufbxi_push_uninit_n(uc, int32_t, mesh_num_indices);
					if (!elem.indices) return 0;

					int32_t *temp_indices = (int32_t*)ufbxi_temp_push_size_uninit(uc,
						UFBXI_TEMP_PRIMARY_RESULT, sizeof(int32_t) * num_indices, __LINE__);
					if (!temp_indices) return 0;
					if (!ufbxi_decode_array(&uc->base, &desc->index_array, temp_indices)) return 0;

					int32_t *vert_ix = mesh->vertex_position.indices;
					for (size_t i = 0; i < mesh_num_indices; i++) {
						int32_t ix = temp_indices[i];
						elem.indices[i] = ix >= 0 && ix < mesh->num_vertices ? vert_ix[ix] : invalid_index;
					}

					ufbxi_temp_pop_size(uc, UFBXI_TEMP_PRIMARY_RESULT, 1, sizeof(int32_t) * num_indices);
				}
				break;

				case UFBXI_ELEMENT_BY_POLYGON_VERTEX:
				{
					// Indexed by polygon vertex: We can use the indices directly,
					// possibly filling the rest with `invalid_index`
					if (num_indices >= mesh_num_indices) {
						elem.indices = ufbxi_push_uninit_n(uc, int32_t, num_indices);
						if (!ufbxi_decode_array(&uc->base, &desc->index_array, elem.indices)) return 0;
					} else {
						elem.indices = ufbxi_push_uninit_n(uc, int32_t, mesh_num_indices);
						if (!ufbxi_decode_array(&uc->base, &desc->index_array, elem.indices)) return 0;
						for (size_t i = num_indices; i < mesh_num_indices; i++) {
							elem.indices[i] = invalid_index;
						}
					}

					// Check that the indices are in range
					for (size_t i = 0; i < num_indices; i++) {
						if (elem.indices[i] < 0 || elem.indices[i] >= (int32_t)num_elements) {
							elem.indices[i] = invalid_index;
						}
					}
				}
				...
				case UFBXI_ELEMENT_BY_ALL_SAME:
				{
					...
					if (!ufbxi_push_generated_indices(uc, p_elem, p_set_elem,
						UFBXI_GENERATED_INDEX_ZERO, mesh_num_indices)) return 0;
				}
				break;

				default:
					return ufbxi_errorf(uc, "Invalid mapping '%s' for vertex element '%s'",
						ufbxi_element_mapping_info_table[desc->mapping].name, info->data_name);
				}
				...
				case UFBXI_ELEMENT_BY_VERTEX:
				{
					// Direct by vertex: If there are enough elements we can re-use
					// the vertex position indices, otherwise we need to truncate it.
					if (num_elements >= mesh->num_vertices) {
						elem.indices = mesh->vertex_position.indices;
					} else {
						elem.indices = ufbxi_push_uninit_n(uc, int32_t, mesh_num_indices);
						if (!elem.indices) return 0;
						for (size_t i = 0; i < mesh_num_indices; i++) {
							int32_t ix = mesh->vertex_position.indices[i];
							elem.indices[i] = ix < (int32_t)num_elements ? ix : invalid_index;
						}
					}
				}
				break;
				...
				case UFBXI_ELEMENT_BY_POLYGON_VERTEX:
				{
					// Direct by polygon vertex: If there's enough elements we can
					// share a consecutive index array, otherwise we need to truncate it.
					if (num_elements >= mesh_num_indices) {
						if (!ufbxi_push_generated_indices(uc, p_elem, p_set_elem,
							UFBXI_GENERATED_INDEX_CONSECUTIVE, mesh_num_indices)) return 0;
					} else {
						elem.indices = ufbxi_push_uninit_n(uc, int32_t, mesh_num_indices);
						if (!elem.indices) return 0;
						for (size_t i = 0; i < num_elements; i++) {
							elem.indices[i] = (int32_t)i;
						}
						for (size_t i = num_elements; i < mesh_num_indices; i++) {
							elem.indices[i] = invalid_index;
						}
					}
				}
			...
			if (p_elem) *p_elem = elem;
			if (p_set_elem) *p_set_elem = elem;

		} else if (info->kind == UFBXI_ELEMENT_KIND_EDGE) {

			char *data = NULL;
			switch (desc->mapping) {

			case UFBXI_ELEMENT_BY_EDGE:
			{
				// Data may be truncated, fill the rest with zeroes. If the data is larger
				// than the number of edges just silently decode a larger buffer.
				if (num_elements >= mesh->num_edges) {
					data = ufbxi_push_uninit_n(uc, char, num_elements * info->element_size);
					if (!data) return 0;
					if (!ufbxi_decode_array(&uc->base, &desc->data_array, data)) return 0;
				} else {
					data = ufbxi_push_uninit_n(uc, char, mesh->num_edges * info->element_size);
					if (!data) return 0;
					if (!ufbxi_decode_array(&uc->base, &desc->data_array, data)) return 0;
					size_t num_truncated = mesh->num_edges - desc->data_array.num_elements;
					memset(data + num_elements*info->element_size, 0, num_truncated*info->element_size);
				}
			...
			case UFBXI_ELEMENT_BY_ALL_SAME:
			{
				// All same: Copy the first element to fill the array
				data = ufbxi_push_uninit_n(uc, char, mesh->num_edges * info->element_size);
				if (!data) return 0;
				if (!ufbxi_decode_array(&uc->base, &desc->data_array, data)) return 0;
				size_t index = 0;
				size_t copy_size = 1;
				while (index < mesh->num_edges) {
					size_t to_copy = mesh->num_edges - index;
					if (to_copy > copy_size) to_copy = copy_size;
					memcpy(data + index*info->element_size, data, to_copy*info->element_size);
					index += to_copy;
					copy_size *= 2;
				}
			}
			...
			default:
				return ufbxi_errorf(uc, "Invalid mapping '%s' for edge element '%s'",
					ufbxi_element_mapping_info_table[desc->mapping].name, info->data_name);
			}

			void **elem = (void**)((char*)mesh + info->offset);
			*elem = data;

		} else if (info->kind == UFBXI_ELEMENT_KIND_FACE) {

			char *data = NULL;
			switch (desc->mapping) {

			case UFBXI_ELEMENT_BY_POLYGON:
			{
				// Data may be truncated, fill the rest with zeroes. If the data is larger
				// than the number of faces just silently decode a larger buffer.
				if (num_elements >= mesh->num_faces) {
					data = ufbxi_push_uninit_n(uc, char, num_elements * info->element_size);
					if (!data) return 0;
					if (!ufbxi_decode_array(&uc->base, &desc->data_array, data)) return 0;
				} else {
					data = ufbxi_push_uninit_n(uc, char, mesh->num_faces * info->element_size);
					if (!data) return 0;
					if (!ufbxi_decode_array(&uc->base, &desc->data_array, data)) return 0;
					size_t num_truncated = mesh->num_faces - desc->data_array.num_elements;
					memset(data + num_elements*info->element_size, 0, num_truncated*info->element_size);
				}
			}
			break;
			...
			case UFBXI_ELEMENT_BY_ALL_SAME:
			{
				...
				size_t index = 1;
				size_t copy_size = 1;
				while (index < mesh->num_faces) {
					size_t to_copy = mesh->num_faces - index;
					if (to_copy > copy_size) to_copy = copy_size;
					memcpy(data + index*info->element_size, data, to_copy*info->element_size);
					index += to_copy;
					copy_size *= 2;
			...
			default:
				return ufbxi_errorf(uc, "Invalid mapping '%s' for face element '%s'",
					ufbxi_element_mapping_info_table[desc->mapping].name, info->data_name);
			}
			...
			*elem = data;
			...
			return ufbxi_errorf(uc, "Internal: Unexpected element kind: %d", info->kind);
		}
...
static int ufbxi_read_objects(ufbxi_context *uc)
{
	...
	size_t num = uc->scene->nodes.size;
...
static int ufbxi_read_root(ufbxi_context *uc)
{
	...
	if (ufbxi_parse_node(uc, root_pos, &root) && root.name.length == 0) {
		...
		uc->focused_node.end_pos = uc->base.size;
	}
	...
	{
		int32_t *generated_indices[UFBXI_NUM_GENERATED_INDEX_TYPES];
		for (int type = 0; type < (int)UFBXI_NUM_GENERATED_INDEX_TYPES; type++) {
			size_t num = uc->max_generated_indices[type];
			if (num > 0) {
				int32_t *indices = ufbxi_push_uninit_n(uc, int32_t, num);
				if (!indices) return 0;
				generated_indices[type] = indices;

				switch ((ufbxi_generated_index_type)type) {
				case UFBXI_GENERATED_INDEX_CONSECUTIVE:
					for (int32_t i = 0; i < num; i++) {
						indices[i] = i;
					}
					break;
				case UFBXI_GENERATED_INDEX_ZERO:
					memset(indices, 0, num * sizeof(int32_t));
					break;
				default:
					return ufbxi_errorf(uc, "Internal: Unhandled generated index type: %d", type);
				}
			} else {
				generated_indices[type] = NULL;
			}
		}

		while (!ufbxi_temp_is_empty(uc, UFBXI_TEMP_GENERATED_INDICES)) {
			ufbxi_generated_index_ptr *ptr = ufbxi_temp_pop(uc, UFBXI_TEMP_GENERATED_INDICES, ufbxi_generated_index_ptr);
			*ptr->p_indices = generated_indices[ptr->type];
		}
	}

	// Gather all typed nodes
	{
		ufbx_scene *scene = uc->scene;
		...
		scene->models.data = ufbxi_push_uninit_n(uc, ufbx_model*, scene->models.size);
		scene->meshes.data = ufbxi_push_uninit_n(uc, ufbx_mesh*, scene->meshes.size);
		if (uc->base.failed) return 0;

		ufbx_model **dst_model = scene->models.data;
		ufbx_mesh **dst_mesh = scene->meshes.data;
		for (size_t i = 0; i < scene->nodes.size; i++) {
...
ufbx_scene *ufbx_load_memory(const void *data, size_t size, const ufbx_load_opts *opts, ufbx_error *error)
{
	...
	if (uc.scene_imp) {
		uc.scene_imp->bc.data = uc.base.data;
		uc.scene_imp->bc.size = uc.base.size;
...
{
	if ((unsigned)type >= UFBX_NUM_PROP_TYPES) return "(bad prop type)";
	return ufbxi_prop_type_info_table[type].name;
}
